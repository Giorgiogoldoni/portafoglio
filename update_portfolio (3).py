#!/usr/bin/env python3
"""
RAPTOR PORTFOLIO OPTIMIZER v2.1
════════════════════════════════
...
v2.1: aggiunto NAV tracking (nav_history.json) con benchmark IWMO + VNGA80.
"""
...
# ── FETCH PREZZI BENCHMARK (semplice, solo ultimo prezzo) ─────────
...
# ── NAV TRACKING (v2.1) ────────────────────────────────────────────
def update_nav_history(base_path: Path, portfolio_list: list, prices_bench: dict, today_str: str):
    """
    Calcola NAV portafoglio (prezzi correnti vs precedenti) e benchmark IWMO/VNGA80.
    Appende un punto a nav_history.json (root del repo).
    """
    NAV_FILE = base_path / "nav_history.json"
...
def run():
    ...
    print("="*60)
    print(f"RAPTOR PORTFOLIO OPTIMIZER v2.1 — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("="*60)
    ...
    prices_bench = fetch_benchmark_prices(["IWMO.MI", "VNGA80.MI"])
